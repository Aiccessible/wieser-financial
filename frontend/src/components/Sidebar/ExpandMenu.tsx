            ...
            <CollapsibleTrigger asChild className="group rounded-sm px-3 py-2  ">
                {isSidebarOpen ? (
                    <div className="flex w-full items-center text-gray-2 ">
                        {icon}
                        <div className={cn('flex w-full transform items-center justify-between duration-300 ease-in')}>
                            <p className={cn('ml-2 text-lg text-gray-2 group-hover:text-white ', {})}>{name}</p>
                            <ChevronRight
                                className={cn(
